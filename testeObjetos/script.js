...
function mostraObjetos() {
  pUsina = window.document.getElementById("pUsina")
  console.log(dadosClientes)
  for (valor of dadosUsina) {
    pUsina.innerHTML += `Dado: ${valor.tempo_h.toFixed(2)}h; ${valor.tensao_V.toFixed(1)}V; ${valor.corrente_A.toFixed(2)}A; ${valor.potencia_kW.toFixed(2)}kW; ${valor.temperatura_C.toFixed(1)}ºC <br>`
  }
  pClientes = window.document.getElementById("pClientes")
  console.log(dadosUsina)
  for (valor of dadosClientes) {
      pClientes.innerHTML += `Cliente: ${valor.numeroCliente} - ${valor.nomeCliente} - ${valor.usinas[0].numeroUsina} - ${valor.usinas[0].percentualUsina} <br>`
  }
}